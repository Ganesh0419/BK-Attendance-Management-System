...
function getExpectedWorkingHours(timingStr) {
  if (!timingStr) return 8; // Default 8 hours
...
async function generateMonthSheet(workbook, monthString, department, usersList, PunchModel) {
  ...
  // Date range: 16th of previous month to 15th of selected month (matching PDF format)
  const prevMonth = mn - 1 === 0 ? 12 : mn - 1;
  const prevYear = mn - 1 === 0 ? yr - 1 : yr;
  const startDate = new Date(prevYear, prevMonth - 1, 16, 0, 0, 0, 0);
  const endDate = new Date(yr, mn - 1, 15, 23, 59, 59, 999);
  ...
  const totalDays = allDates.length; // typically 28-31 days
  ...
  const sheetName = `${startDate.toLocaleDateString('en-US', { month: 'short' })}-${endDate.toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}`.replace(/[\\*\\?\\/\\\\\\[\\]]/g, '');
  const lastCol = totalDays + 1; // column 1 = label, columns 2..totalDays+1 = days
  const lastColLetter = getColLetter(lastCol);
  ...
  // ========== ROW 5: Date Range (centered) ==========
  worksheet.mergeCells(`A5:${lastColLetter}5`);
  worksheet.getCell('A5').value = `${startLabel.replace(/ /g, '-')} To ${endLabel.replace(/ /g, '-')}`;
  worksheet.getCell('A5').font = { bold: true, size: 12 };
  worksheet.getCell('A5').alignment = { horizontal: 'center' };

  // ========== ROW 6: Generated On (right aligned) ==========
  worksheet.mergeCells(`A6:${lastColLetter}6`);
  const genDate = new Date();
  worksheet.getCell('A6').value = `Generated On: ${genDate.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })} ${genDate.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true })}`;
  worksheet.getCell('A6').font = { bold: true, size: 10 };
  worksheet.getCell('A6').alignment = { horizontal: 'right' };

  // ========== ROW 7: Day Header (Day, Day1, Day2, ...) ==========
  const dayRowValues = ['Day'];
  for (let i = 0; i < totalDays; i++) {
    dayRowValues.push(`Day${i + 1}`);
  }

  // ========== ROW 8: Days with date + weekday ==========
  ...
  for (let i = 0; i < totalDays; i++) {
    ...
    dateRowValues.push(`${dayNum}-${monthShort}\n${weekday}`);
  }
  ...
  dateRow.font = { bold: true, size: 8 };
  dateRow.alignment = { wrapText: true, vertical: 'middle', horizontal: 'center' };
  dateRow.height = 28;
  currentRow++;
  ...
  users.forEach(user => {
    ...
    const thirdCol = Math.ceil(totalDays / 3);
    const twoThirdCol = Math.ceil(2 * totalDays / 3);

    // Employee Code
    const codeEndLetter = getColLetter(thirdCol + 1);
    worksheet.mergeCells(`A${currentRow}:${codeEndLetter}${currentRow}`);
    ...
    // Employee Name
    const nameStartLetter = getColLetter(thirdCol + 2);
    const nameEndLetter = getColLetter(twoThirdCol + 1);
    worksheet.mergeCells(`${nameStartLetter}${currentRow}:${nameEndLetter}${currentRow}`);
    worksheet.getCell(`${nameStartLetter}${currentRow}`).value = `Employee Name:- ${user.name}`;
    worksheet.getCell(`${nameStartLetter}${currentRow}`).font = { bold: true, size: 9 };

    // Designation
    const desigStartLetter = getColLetter(twoThirdCol + 2);
    worksheet.mergeCells(`${desigStartLetter}${currentRow}:${lastColLetter}${currentRow}`);
    worksheet.getCell(`${desigStartLetter}${currentRow}`).value = `Designation - ${user.role ? (user.role.charAt(0).toUpperCase() + user.role.slice(1)) : 'N/A'}`;
    worksheet.getCell(`${desigStartLetter}${currentRow}`).font = { bold: true, size: 9 };

    currentRow++;

    // --- Day header row (repeated per user as in PDF) ---
    const uDayRow = worksheet.getRow(currentRow);
    uDayRow.values = dayRowValues;
    uDayRow.font = { bold: true, size: 9 };
    currentRow++;

    // --- Build punch lookup by date string ---
    const userPunches = punchesByUser[String(user.fingerprint_id)] || punchesByUser[String(user.id)] || [];
    const punchesByDateStr = {};
    userPunches.forEach(p => {
      const dStr = `${p.timestamp.getFullYear()}-${String(p.timestamp.getMonth() + 1).padStart(2, '0')}-${String(p.timestamp.getDate()).padStart(2, '0')}`;
      if (!punchesByDateStr[dStr]) punchesByDateStr[dStr] = [];
      punchesByDateStr[dStr].push(p);
    });
    ...
    for (let i = 0; i < totalDays; i++) {
      ...
      const dayPunches = (punchesByDateStr[dStr] || []).sort((a, b) => a.timestamp - b.timestamp);
    ...
    // Write data rows
    const inRow = worksheet.getRow(currentRow);
    inRow.values = inTimes;
    inRow.font = { size: 8 };
    inRow.getCell(1).font = { bold: true, size: 8 };
    currentRow++;

    const outRow = worksheet.getRow(currentRow);
    outRow.values = outTimes;
    outRow.font = { size: 8 };
    outRow.getCell(1).font = { bold: true, size: 8 };
    currentRow++;

    const durRow = worksheet.getRow(currentRow);
    durRow.values = durations;
    durRow.font = { size: 8 };
    durRow.getCell(1).font = { bold: true, size: 8 };
    currentRow++;

    const statRow = worksheet.getRow(currentRow);
    statRow.values = statuses;
    statRow.font = { size: 8 };
    statRow.getCell(1).font = { bold: true, size: 8 };
    // Color-code status cells
    for (let c = 2; c <= totalDays + 1; c++) {
      const val = statRow.getCell(c).value;
      if (val === 'P') {
        statRow.getCell(c).font = { size: 8, bold: true, color: { argb: 'FF008000' } };
      } else if (val === 'A') {
        statRow.getCell(c).font = { size: 8, bold: true, color: { argb: 'FFFF0000' } };
      } else if (val === 'WO') {
        statRow.getCell(c).font = { size: 8, bold: true, color: { argb: 'FF888888' } };
      }
    }
    currentRow++;
  });
